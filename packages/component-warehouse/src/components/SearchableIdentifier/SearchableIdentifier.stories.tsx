...
import { SearchableIdentifier } from './SearchableIdentifier';
import type { SearchResult } from './SearchableIdentifier';
import { Card, Space } from 'antd';
import { useState } from 'react';

const meta: Meta<typeof SearchableIdentifier> = {
  title: 'Components/SearchableIdentifier',
  component: SearchableIdentifier,
  parameters: {
    layout: 'centered',
  },
  tags: ['autodocs'],
...
export default meta;
type Story = StoryObj<typeof SearchableIdentifier>;
 ...
 * This matches the signature expected by SearchableIdentifier component
 */
...
export const CustomDebounce: Story = {
  render: (args) => {
    ...
    return (
      <Card style={{ width: 600, padding: '20px' }}>
        <div style={{ marginBottom: '16px', padding: '8px', background: '#fff7e6', borderRadius: '4px', fontSize: '12px' }}>
          Debounce delay: 500ms (slower response for demonstration)
        </div>
        <SearchableIdentifier
          {...args}
          value={value}
...
export const Interactive: Story = {
  render: () => {
    ...
    const [identifierType, setIdentifierType] = useState<'isin' | 'sedol'>('isin');
    const [selectedResult, setSelectedResult] = useState<SearchResult | null>(null);
    const [searchHistory, setSearchHistory] = useState<Array<{ query: string; result: SearchResult | null }>>([]);

    return (
      <Card style={{ width: 800, padding: '20px' }}>
        <Space direction="vertical" style={{ width: '100%' }} size="large">
          <div>
            <h3 style={{ marginBottom: '16px' }}>Interactive SearchableIdentifier</h3>
            <SearchableIdentifier
              value={value}
              onChange={setValue}
              onSelect={(val, result) => {
